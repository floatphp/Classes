...
class CSV
{
    ...
    public function save($file = null, $data = array(), $append = false, $fields = array())
    {
        ...
        return $this->write($file, $this->unparse($data, $fields, $append, $is_php), $mode);
    }
    /**
     * Output
    ...
    public function parse_string($data = null)
    {
        ...
        if (!empty($this->sortBy)) {
            ...
            ($this->sortReverse) ? krsort($rows, $sort_type) : ksort($rows, $sort_type);
    ...
    public function load_data($input = null) {
        ...
        if (!empty($data) || $data = $this->read($file)) {
            if ($this->file != $file) {
                $this->file = $file;
            }
    ...
    protected function _enclose_value($value = null, $delimiter = null) {
        ...
        if ($value !== null && $value != '') {
            ...
            if (preg_match("/" . $delimiter_quoted . "|" . $enclosure_quoted . "|\n|\r/i", $value) || ($value{0} == ' ' || substr($value, -1) == ' ') || $this->encloseAll) {
     ...
     */
    protected function read($file = null)
    {
        if (is_readable($file)) {
            if (!($fh = fopen($file, 'r'))) {
                return false;
     ...
     * @return  true or false
     */
    protected function write($file, $string = '', $mode = 'wb', $lock = 2)
    {
